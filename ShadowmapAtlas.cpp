...
namespace AZ
{
    namespace Render
    {
        ...
        {
            assert(m_requireFinalize/*, "Initialize before set shadowmap size"*/);
            m_requireFinalize = true;
            m_indicesForSize[size].push_back(index);
            m_baseShadowmapSize = std::max(size, m_baseShadowmapSize);
        }

        void ShadowmapAtlas::Finalize()
        {
            assert(m_requireFinalize/*, "Initialize before finalization."*/);
                ...
                size = static_cast<ShadowmapSize>(static_cast<uint32_t>(size) / 2))
            {
                ...
                if (m_indicesForSize.find(size) != m_indicesForSize.end())
                {
                    for (const size_t index : m_indicesForSize[size])
                    {
                        ...
                        m_maxArraySlice = std::max(m_maxArraySlice, currentLocation[0]);
            ...
            assert(m_shadowmapIndexNodeTree.empty() || GetNodeOfTree(Location{}).size() == GetArraySliceCount(),
        ...
        /*Data::Instance<RPI::Buffer>*/void* ShadowmapAtlas::CreateShadowmapIndexTableBuffer(const std::string& bufferName) const
        {
        ...
        const std::vector<ShadowmapAtlas::ShadowmapIndexNode> &ShadowmapAtlas::GetShadowmapIndexTable() const
        {
            assert(!m_requireFinalize/*, "Finalization is required."*/);
        ...
        void ShadowmapAtlas::BuildIndexTableData()
        {
            ...
            std::unordered_map<Location, uint32_t, LocationHasher> indices;
            { // scope to avoid variable name pollution.
                ...
                assert(nodes.size() == nonRootSubtableCount/*, "subtable count has an unexpected value."*/);
            ...
            for (const auto& it : m_shadowmapIndexNodeTree)
            {
                ...
                for (uint8_t digit = 0; digit < digitCount; ++digit)
                {
                    ...
                    else
                    {
                        ...
                        m_indexTableData[indexInTable].m_shadowmapIndex = static_cast<uint32_t>(indicesInNode[digit]);
